...
- The "PSX Announcements" tile was asked to update "daily at 9:30 AM and 3:30 PM" — the dashboard has no clock-triggered refresh mechanism (every other feature just polls on a fixed `setInterval`), so this was implemented as hourly polling (`PSX_ANNOUNCEMENTS_POLL_INTERVAL_MS` in `app/page.tsx`) plus a matching `revalidate: 3600` on the server route, which naturally covers both times without needing new architecture. If literal clock-time-triggered refreshes are ever wanted, that's a different mechanism (e.g. a cron-triggered rebuild) than anything else on this dashboard uses.
