...
- `app/page.tsx` — client dashboard: polls `/api/oil-prices` every 5 min (PSO cards + price history chart via Recharts), `/api/mari-gas-price` every 30 min (Mari gas price cards + OGRA status panel), `/api/mari-share-price` every 5 min, `/api/commodity-prices` every 30 min (Dubai Crude / JKM LNG tiles), `/api/global-oil-benchmarks` every 30 min (the 6-benchmark tile), and `/api/psx-announcements` every hour (the PSX news tile). `RECEIVABLES_BY_QUARTER`, `OIL_IMPORTS_LAST_MONTH`, and `IMF_PROGRAM` are hardcoded constants (same manual-verification pattern as `LAST_VERIFIED_MARI_PRICE`) — update by hand when a newer quarterly report, OCAC monthly row, or IMF review is read.
